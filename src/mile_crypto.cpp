...
void SignMessage(const vector<unsigned char>& message, const PrivateKey& privateKey, Signature& signature)
{
    PublicKey publicKey;
...
Signer::Signer(const PrivateKey &privateKey)
    ...
    , m_publicKey()
{
    RestoreKeyPairFromPrivate(m_privateKey, m_publicKey);
}
...
void Signer::Dump(std::ostream& outputStream)
{
