...
bool DecodeBase58Check(const char* psz, vector<unsigned char>& vchRet)
{
    ...
    //
    uint_least32_t crc32 = Crc32(&vchRet[0], sz - 4);
    //
    if (static_cast<unsigned char>(crc32 & 0xff) != vchRet[sz - 4] ||
        static_cast<unsigned char>((crc32 >> 8) & 0xff) != vchRet[sz - 3] ||
        static_cast<unsigned char>((crc32 >> 16) & 0xff) != vchRet[sz - 2] ||
        static_cast<unsigned char>((crc32 >> 24) & 0xff) != vchRet[sz - 1])
    {
        vchRet.clear();
        return false;
    }
